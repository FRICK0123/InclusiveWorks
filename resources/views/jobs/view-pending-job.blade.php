        ...
        <div class="container">
            <div class="p-3 border border-1 border-dark">
                <div class="container-fluid d-flex justify-content-between">
                    <img src="{{asset('uploads/employers/' . $jobInfo['company-logo'])}}" alt="Company Logo" class="img-fluid" width="50" style="border-radius: 50%;">
                    <label class="mt-3">Posted {{$jobInfo['created_at']->diffForHumans()}}</label>
                </div>
                            
                <span class="fw-bold">{{$jobInfo['company-name']}}</span>

                <h2 class="mt-3 fw-bold">{{$jobInfo['job-title']}}</h2>
                <label class="mt-3 fw-bold">Job Description:</label>
                <p>{{$jobInfo['job-description']}}</p>

                <label class="mt-3 fw-bold">Salary Range:</label>
                <p>₱{{$jobInfo['salary-range-start']}} to ₱{{$jobInfo['salary-range-end']}} {{$jobInfo['salary-frequency']}}</p>

                <label class="mt-3 fw-bold">Employment Type:</label>
                <p>{{$jobInfo['employment-type']}}</p>

                <label class="mt-3 fw-bold">Education Level Required:</label>
                <p>Must be a {{$jobInfo['education-level']}} graduate</p>

                <label class="mt-3 fw-bold">Experience Level:</label>
                <p>{{$jobInfo['experience-level']}}</p>

                <label class="mt-3 fw-bold">Must Have Skills:</label>
                <p>{{$jobInfo['skills']}}</p>

                <label class="mt-3 fw-bold">Capable Person with Disabilities</label>
                <p>{{$jobInfo['disablities']}}</p>
